...
class LearningAgent(Agent):
	...
	if testing:
		...
		self.t +=1
			if self.t<100:
				...
				self.epsilon=self.epsilon=math.exp(-0.006*(self.t-99))	return None
	...
	# since there could be more than one max Q-value, it is necessary to log all actions with a maximal Q in a list and pick a 	#random choice
	if not self.learning:
   	...
   	if self.learning:
		if self.epsilon>random.random():
			...
			for entry in self.Q[state]:
				if maxQ == self.Q[state][entry]:
					best_action.append(entry)
